...
def images_upload():
    ...
    for upload in request.files.getlist("file"):
        ...
        query2 = "INSERT INTO user_has_images (user_id, images_name) VALUE(%s, %s)"
        ...
        #                    VALUES (%s,%s)'''
        #
        # cursor.execute(query3, (user_id, row[0]))
        #
        # cnx.commit()
        ...
        #         thumb.format = "png"
        #         thumb.save(path = path_thumb_full)
...
def auto_upload():
    ...
    imagelist = sorted(glob.glob('test_images/' + test_image_name))

    ROOT = os.path.join(os.path.dirname(test_image_name), 'images', str(user_id))
