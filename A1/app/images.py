...
def images_upload():
    ...
    query = '''SELECT image_name FROM user_has_images WHERE user_id = %s'''
    ...
    if len(upload_list) == 0:
        ...
        for upload in request.files.getlist("file"):
            ...
            # check if browse is not selected
            if filepath == "":
                flag = 2
                break
            # check if the img_name already exist in the database
            if any(filepath in s for s in img_saved):
                flag = 1
                break
            path = os.path.join(ROOT, filepath)
            print("Auto path: " + path + "\n")
            print("Auto filepath: " + filepath + "\n")
...
def auto_upload():
    ...
    return redirect(url_for('user_home',repeat=repeat))
